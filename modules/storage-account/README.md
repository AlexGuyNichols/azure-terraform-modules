...
| <a name="input_account_tier"></a> [account\_tier](#input\_account\_tier) | Performance tier for the storage account. Only 'Standard' is supported: the module's implicit account\_kind (StorageV2) would make 'Premium' a page-blob-only account, which cannot host block blobs (remote state) and rejects blob versioning at apply time. | `string` | `"Standard"` | no |
